...
void BowLady::snipe(QList<void *> args)
{
    ...
    {
        this->engine->drawCards(5 - dst->getHandCardNum(),NOTHARMED,dst,this);
    }
...
void AnSha::FanShi(QList<void*> args)
{
    ...
    engine->drawCards(1,0,(PlayerEntity*)args[0],this);
...
void MoJian::HeiAnZhenChan2(QList<void*> args)
{
    if(this != (PlayerEntity*)args[0]||!isHeiAnZhenChan)
        return;
    isHeiAnZhenChan=false;
    engine->drawCards(getHandCardMax()-getHandCardNum(),0,this,this);
}
...
void ZhongCai::PanJueTianPing(QList<void *> args)
{
    ...
    if(magic->infor2)
    {
        ...
        engine->drawCards(drawNum,0,this,this);
        teamArea.setGem(color,teamArea.getGem(color)+1);
        coder.stoneNotice(color,teamArea.getGem(color),teamArea.getCrystal(color));
    }
    else
...
void ShenGuan::ShenShengQiYue(QList<void *> args)
{
    ...
    this->subCrossNum(start.infor3);
    coder.crossChangeNotice(this->getID(), getCrossNum());
    PlayerEntity* dst = engine->getPlayerByID(start.dstID);
    dst->addCrossNum(start.infor3, 4);
...
void XianZhe::ShengJieFaDian(QList<void *> args)
{
    ...
    int i=0;
    int j=0;
    for(i=0;i<magic->infor2;i++)
        cards.append(getCardByID(cardNum[i].toInt()));
    for(j=0;j<magic->infor3;j++)
...
void WuNv::makeConnection(BackgroundEngine *engine)
{
    ...
    connect(engine,SIGNAL(trueLoseMoraleSIG(int,int*,PlayerEntity*,PlayerEntity*)),this,SLOT(ToLiuXueXingTai(int,int*,PlayerEntity*)));
...
void WuNv::TongShengGongSi(QList<void *> args)
{
    ...
    engine->drawCards(2, 0, this,this);
...
{
    connect(engine,SIGNAL(trueLoseMoraleSIG(int,int*,PlayerEntity*,PlayerEntity*)),this,SLOT(LingHunTunShi(int,int*,PlayerEntity*)));
...
void LingHun::LingHunJingXiang(QList<void *> args)
{
    ...
    if(dst->getHandCardMax()-dst->getHandCardNum()>=3)
        engine->drawCards(drawNum,0,dst,this);
    else
    {
...
void HongLian::makeConnection(BackgroundEngine *engine)
{
    ...
    connect(engine,SIGNAL(trueLoseMoraleSIG(int,int*,PlayerEntity*,PlayerEntity*)),this,SLOT(ToReXueFeiTeng(int,int*,PlayerEntity*)));
...
void HongLian::XueXingDaoYan(QList<void *> args)
{
    ...
    if(start.infor2 == 0||(start.infor3+start.infor5)<=0)
        return;
    this->subCrossNum(start.infor3+start.infor5);
    coder.crossChangeNotice(this->getID(),crossNum);
    ...
    PlayerEntity* dst = engine->getPlayerByID(start.dstID);
    dst->addCrossNum(start.infor3);
    coder.crossChangeNotice(start.dstID,dst->getCrossNum());
    if(start.infor4!=-1)
    {
        dst = engine->getPlayerByID(start.infor4);
        dst->addCrossNum(start.infor5);
...
{
    if(this != ((PlayerEntity*)args[0])||this->getEnergy() <= 0||!tap||JieJiaoJieZaoCounted)
        return;
    engine->addActionNum(ATTACKORMAGIC);
    JieJiaoJieZaoCounted=true;
}
...
//魔枪
MoQiang::MoQiang(BackgroundEngine *engine, int id, int color):PlayerEntity(engine,id,color)
{
    this->characterID=29;
    this->star=4;
    JieFangUsed=false;
...
void MoQiang::makeConnection(BackgroundEngine *engine)
{
    connect(engine,SIGNAL(actionPhaseSIG(QList<void*>)),this,SLOT(AnZhiJieFang1(QList<void*>)));
    connect(engine,SIGNAL(timeLine2hitSIG(QList<void*>)),this,SLOT(AnZhiJieFang2(QList<void*>)));
    connect(engine,SIGNAL(timeLine2missedSIG(QList<void*>)),this,SLOT(AnZhiJieFang3(QList<void*>)));
    connect(engine,SIGNAL(actionPhaseSIG(QList<void*>)),this,SLOT(HuanYingXingChen(QList<void*>)));
    connect(engine,SIGNAL(trueLoseMoraleSIG(int,int*,PlayerEntity*,PlayerEntity*)),this,SLOT(HuanYingXingChen2(int,int*,PlayerEntity*,PlayerEntity*)));
    connect(engine,SIGNAL(timeLine3SIG(QList<void*>)),this,SLOT(AnZhiBiZhang(QList<void*>)));
    connect(engine,SIGNAL(skillMagic(QList<void*>)),this,SLOT(ChongYing1(QList<void*>)));
    connect(engine,SIGNAL(additonalActionSIG(QList<void*>)),this,SLOT(ChongYing2(QList<void*>)));
    connect(engine,SIGNAL(timeLine2hitSIG(QList<void*>)),this,SLOT(ChongYing3(QList<void*>)));
    connect(engine,SIGNAL(timeLine2missedSIG(QList<void*>)),this,SLOT(ChongYing4(QList<void*>)));
    connect(engine,SIGNAL(timeLine2hitSIG(QList<void*>)),this,SLOT(QiHeiZhiQiang(QList<void*>)));
    connect(engine,SIGNAL(turnBeginPhaseSIG(QList<void*>)),this,SLOT(skillReset(QList<void*>)));
}

void MoQiang::AnZhiJieFang1(QList<void *> args)
{
    if(this != (PlayerEntity*)args[0])
        return;
    if(JieFangUsed || StartUsed)
        return;
...
void MoQiang::AnZhiJieFang3(QList<void *> args)
{
    if(this != (PlayerEntity*)args[0])
        return;
    if(!JieFangUsed || !JieFangFirst)
...
void MoQiang::HuanYingXingChen(QList<void *> args)
{
    if(this != (PlayerEntity*)args[0])
        return;
    if(StartUsed || !JieFangUsed)
        return;
    BatInfor huanying;
    coder.askForSkill(id,"幻影星辰");
    huanying=messageBuffer::readBatInfor();

    BatInfor huanying2;
    if(huanying.reply==0)
        return;
    if(huanying.infor1==0)
    {
        Harm harm;
        harm.harmPoint=2;
        harm.type=MAGICHARM;
    ...
    if(HuanYingUsed){
        coder.askForSkill(id,"幻影星辰",QString::number(1));
        huanying2=messageBuffer::readBatInfor();
    }
    if(huanying2.infor1==1 && HuanYingUsed)
    {
        PlayerEntity* dst=engine->getPlayerByID(huanying2.dstID);
        Harm harm;
        harm.harmPoint=2;
        harm.type=MAGICHARM;
...
void MoQiang::HuanYingXingChen2(int harmed, int *howMany, PlayerEntity *dst,PlayerEntity*src)
{
    if(!HuanYingUsed||dst!=this||src!=this||*howMany==0)
        return;
    HuanYingUsed=false;
}

void MoQiang::AnZhiBiZhang(QList<void *> args)
{
    int howMany;
    if(this != ((PlayerEntity*)args[1])||handCards.size()==0)
        return;
    coder.askForSkill(this->getID(),"暗之壁障");
    QString msg=messageBuffer::readMsg();
    QStringList arg=msg.split(";");
    if(arg[1].toInt()==0)
        return;
    ...
    else
    {
        ...
        this->removeHandCards(cards,true);
        coder.notice("魔枪发动【暗之壁障】");
        coder.discardNotice(this->getID(),howMany,"y",cards);
    }
}

void MoQiang::QiHeiZhiQiang(QList<void *> args)
{
    if(this != (PlayerEntity*)args[0])
        return;
    if(!*(bool*)args[4])
    ...
    if(!JieFangUsed || FirstTrun)
        return;
    coder.askForSkill(id,"漆黑之枪");
    BatInfor bat=messageBuffer::readBatInfor();
    if(bat.reply==0)
        return;
    int n=bat.infor1;
    if(myself->getCrystal()>=n)
        setCrystal(crystal-n);
    else
    {
        n-=myself->getCrystal();
        setCrystal(0);
...
void MoQiang::ChongYing1(QList<void *> args)
{
    BatInfor *magic = (BatInfor*)args[0];
    if(magic->srcID != id || magic->infor1 != 2905 || JieFangFirst)
        return;
    QList<CardEntity*>cards;
    cards<<getCardByID(magic->CardID);
    this->removeHandCards(cards,true);
    ...
    {
        coder.askForChongYing(ptr->getID(),color);
        BatInfor ans=messageBuffer::readBatInfor();
        if(ans.reply==1){
        cards.clear();
        cards<<getCardByID(ans.CardID);
        ptr->removeHandCards(cards,true);
        coder.discardNotice(ptr->getID(),1,"y",cards);
        if(ans.infor1==1)
            AddAttackPoint++;
        }
    }
...
void MoQiang::ChongYing2(QList<void *> args)
{
    BatInfor *skill = (BatInfor*)args[0];
    if(id != skill->srcID||skill->infor1!=2906)
        return;
    coder.notice("魔枪发动【充盈】的额外攻击效果");
}

void MoQiang::ChongYing3(QList<void *> args)
{
    if(this != (PlayerEntity*)args[0])
        return;
    if(!*(bool*)args[4])
    ...
    if(!ChongYingUsed)
        return;
    Harm* harm=(Harm*)args[2];
    harm->harmPoint+=AddAttackPoint;
    ChongYingUsed=false;
}

void MoQiang::ChongYing4(QList<void *> args)
{
    if(this != (PlayerEntity*)args[0])
        return;
    if(!*(bool*)args[4])
...
void MoQiang::skillReset(QList<void *> args)
{
    if(this != (PlayerEntity*)args[0])
        return;
    StartUsed=false;
...
void JianDi::skillReset(QList<void *> args)
{
    if(this != (PlayerEntity*)args[0])
        return;
    BuQuCounted=false;
...
{
    if(this != (PlayerEntity*)args[0]||BuQuCounted)
        return;
    if(this->getEnergy()==0)
        return;    
    engine->addActionNum(ATTACK);
...
void JianDi::BuQuYiZhi2(QList<void *> args)
{
    ...
    engine->drawCards(1,0,this,this);
    setToken(0,token[0]+1);
    coder.tokenNotice(id,0,token[0]);
}
//蝶舞 24
...
{
    connect(engine,SIGNAL(timeLine1SIG(QList<void*>)),this,SLOT(MoGuanChongJi(QList<void*>)));
...
{
    if(this != ((PlayerEntity*)args[0])||coverCards.size()==0||ChongNengUsed||DuoChongSheJiUsed)
        return;
    PlayerEntity* dst=(PlayerEntity*)args[1];
    if(dst->getHandCardNum()>=dst->getHandCardMax())
        return;
    coder.askForSkill(id,"魔贯冲击");
    BatInfor ans = messageBuffer::readBatInfor();
    if(ans.reply == 0)
        return;
    CardEntity *chongNeng = getCardByID(ans.CardID);
    QList<CardEntity*> chongNengs;
    chongNengs << chongNeng;
    engine->moveCardFromCoverToDiscard(chongNeng,true);
    ...
    this->setToken(2,this->getCoverCards().count());
    coder.tokenNotice(this->getID(),2,this->getCoverCards().count());
    coder.coverCardNotice(this->getID(),1,chongNengs,true,true);
}
...
{
    if(this != ((PlayerEntity*)args[0])||coverCards.size()==0||MoGuanChongJiUsed||DuoChongSheJiCounted)
        return;
    engine->addActionNum(ATTACK);
    DuoChongSheJiCounted=true;
}
...
void MoGong::ChongNengMoYan(QList<void *> args)
{
    ...
    if(start.reply == 1)
    {
        ...
        engine->drawCards(start.infor3,0,this,this);
    ...
    else
    {
        ...
        PlayerEntity* dst;
        if(start.dstID==-1)
            engine->drawCards(3,0,this,this);
        else
        {
            dst = engine->getPlayerByID(start.dstID);
            if(dst->getHandCardNum()>0)
                coder.askForDiscard(start.dstID,1,NULL);
            QList<CardEntity*> cardChosen;
            cardChosen = messageBuffer::readCardID(1);
